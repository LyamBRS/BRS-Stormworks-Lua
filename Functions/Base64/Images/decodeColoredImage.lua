...
-- `26/03/13`
-- ### Description
-- Decodes colored base64 string encoded images, into drawable images.
-- This is the full function. Ensure your decoder functions ALWAYS WORK FOR THESE IMAGES.
--
-- ### Used for
...
-- ### @Returns
-- ##### 1. `image` : `table`
-- {{{r,g,b},{{x,y,w,h}, {x,y,w,h}}}, {{r,g,b}, {{x,y,w,h}}}}
function decodeImage(encoded)
    -- [BRS] - Seperates the color definitions from their rectangle usages
    colorsSplit = split(encoded, "@")

    -- [BRS] - Color parsing. If you're using BRS Themes, you could reduce character usage by making this portion a function.
    colors = {}
    for i=1, #colorsSplit[1], 3 do
        color = colorsSplit[1]:sub(i, i+2)
        temporary = {}
        for j=1, 3 do
            temporary[j] = ((numberFromBase64(color:sub(j,j))-1)/63)*255
        end
        colors[#colors+1] = temporary
    end

    -- [BRS] - Creating the bundles of rectangles based on the color they use.
    image = {}
    -- [BRS] - Bundle separation, color:xywhxywhxywh
	for _,v in pairs(split(colorsSplit[2],"?")) do
		i = #image+1
		bundle = split(v,":")
		image[i] = {colors[numberFromBase64(bundle[1])],{}}
